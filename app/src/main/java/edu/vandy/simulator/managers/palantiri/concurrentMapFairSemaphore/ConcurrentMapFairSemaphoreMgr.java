...
public class ConcurrentMapFairSemaphoreMgr
    ...
    public Palantir acquire() throws CancellationException {
        ...
        Iterator<Palantir> iterator = mPalantiriMap.keySet().iterator();
        while(iterator.hasNext())
        {
            Palantir palantir = iterator.next();
            if(palantir == null)
                iterator = mPalantiriMap.keySet().iterator();
            else if(mPalantiriMap.replace(palantir,true,false))
                return palantir;
        }
        return null;
    }
